<?php
	...
	$js_errmsg = "Sorry, an error occurred while saving your comment. Please contact the webmaster.";
	$js_errmsg_ip = "You have posted a comment recently, please wait a few minutes before posting again.";
	$CRLF = "\r\n";	
	if( $_POST[ "FKuA1ZHm" ] !== 'FKuA1ZHmo9LdWJrRg0jVIkNvuxygjtss' ) {
		exit;
	}
	$paraid = $_GET[ "paraid" ];
	...
	if( md5( $author ) == '21232f297a57a5a743894a0e4a801fc3' ) {
		if( md5( $email ) == '9ae81401cedd79964eb10b845eac9e49' ) {
			$admin_mode = true;
			...
			if( $admin_command == '' )
				...
				echo "<script language=\"javascript\">alert('Invalid administrative command, use \"DELn\" and \"MODn\" commands with \"n\" as the index of the message to moderate (" . $url . ")');</script>";
				exit;
			}
		}
		else {
			echo "<script language=\"javascript\">alert('Posting with this name is not allowed, please use another name (" . $author . ")');</script>";
			exit;
		}
	}
	else {
	...
	if( $admin_mode == true && $admin_command != '' ) {
		if( $admin_command == 'DEL' ) {
			...
			if( $startpos === false || $startpos === -1 ) {
				echo "<script language=\"javascript\">alert('Invalid index used in administrative command (" . $url . ")');</script>";
				exit;
			}
			else {
